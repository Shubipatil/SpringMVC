<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
<style type="text/css">
body {
	background-image: url("downloads/Hmimg.jsp");
}
</style>
</head>
<body align="center"
	style="background-image: url(data:image/jpeg;base64,); background-size: 100vw 100vh;background-repeat: no-repeat;
	background-position:;">
	<!-- In this JSp file we have two anchor tags with 2 href  -->
	<h1 style="color: red">Welcome to spring MVC</h1>
	<hr>

	<!-- when we click on " add student " link it will go in search of @RequestMapping ("/register") 
	 in our controller  class -->

	<a href="register">Add Student</a>
	<br>
	<br>

	<!-- when we click on " display Students " link it will go in search of @RequestMapping ("/display") 
	 in our controller  class -->
	<a href="display">Display Student</a>

</body>
</html>